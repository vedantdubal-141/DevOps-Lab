...
_Exploring the inner workings of Linux from boot to user space._
...
Most users install an OS and stop at "it boots." But what _actually_ happens between pressing the power button and reaching the login prompt? This guide breaks down the hidden complexity beneath modern systems.
...
_UEFI → Bootloader → Kernel → initramfs → systemd → User Space_
...
- **Alternative: `systemd-boot`**
  systemd-boot is a simpler UEFI bootloader that loads the kernel directly, but most distributions still rely on initramfs for early hardware support.
...
> **Fun Fact:**
> MacOS is also based on Unix, Unix-alike system like MINIX which inspired Linus Torvalds to create Linux from scratch in C. Both systems share core traits like:
>
